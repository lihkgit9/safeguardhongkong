$(function () {
    ...
    sub.onclick = function (e) {
        ...
        if (username == '') {
            dialog('Please input your Name.');
        }
        if (hkid == '') {
            dialog('Please input the first 4 digits of your ID No.');
        }
        if (yzm == "") {
            dialog("Please input verification code.");
            return;
        }
    ...
    function submitHandle(o) {
        ...
        xhr.open('POST', 'submit_eng.php', true);
    ...
    function success(e) {
        ...
        try {
            ...
            dialog('An unknown error occurred. Please try again later.');
        }

        switch (data.errorcode) {
            case 0:
                dialog(data.message, function () {
                    location.href = 'sign_eng.html';
                });
                break;
            case -43:
                dialog(data.message, function () {
    ...
    function dialog(msg, fn) {
        ...
        div.innerHTML =
            '<div class="dialog-header"><b id="dialog-btn-close">×</b></div>' +
            '<div class="dialog-body">' + (msg || '') + '</div>' +
            '<div class="dialog-footer"><span id="dialog-btn-ok">OK</span></div>';
